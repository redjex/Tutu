...
function renderPropertyInfo(index = state.inspectedIndex ?? state.position) {
  ...
  const rentMarkup = [1, 2, 3, 4, 5].map((level) => `<span class="property-info__rent-stars" aria-label="${level} звёзд">${Array.from({ length: level }, () => '<img src="/assets/images/star.svg" alt="" />').join('')}</span><span>${formatMoney(Math.round(tile.purchasePrice * level / 5))}</span>`).join('');
